...
{
    public KeyValueController(ISender mediator)
        : base(mediator) { }

    [HttpGet]
    public async Task<IActionResult> GetAll([FromQuery] int selectedEtcdConnectionId)
    {
        var query = new GetAllQuery() { EtcdConnectionId = selectedEtcdConnectionId };
    ...
    {
        var query = new GetAllKeysQuery() { EtcdConnectionId = selectedEtcdConnectionId };
        var result = await _mediator.Send(query);
        return Ok(result);
    }
    ...
    public async Task<IActionResult> GetByKey(
        ...
        [FromQuery] int selectedEtcdConnectionId
    )
    {
        var query = new GetByKeyQuery() { Key = key, EtcdConnectionId = selectedEtcdConnectionId };
        var result = await _mediator.Send(query);
        return Ok(result);
    }

    [HttpGet("GetByKeyPrefix")]
    public async Task<IActionResult> GetByKeyPrefix(
    ...
    public async Task<IActionResult> Save(
        [FromBody] SaveKeyValueModel model,
        [FromQuery] int selectedEtcdConnectionId
    )
    {
    ...
    public async Task<IActionResult> Delete(
        [FromQuery] string key,
        [FromQuery] int selectedEtcdConnectionId,
        [FromQuery] bool deleteRecursive = false
    )
    {
    ...
    [HttpPost("RenameKey")]
    public async Task<IActionResult> RenameKey(
        [FromBody] RenameKeyModel model,
        [FromQuery] int selectedEtcdConnectionId
    )
    {
    ...
    public async Task<IActionResult> GetRevisionDetail(
        [FromQuery] string key,
    ...
    public async Task<IActionResult> ImportNodes(
        [FromBody] KeyValue[] KeyValues,
        [FromQuery] int selectedEtcdConnectionId
    )
    {
